...
# --- ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ БЕЗОПАСНОЙ ОТПРАВКИ МЕДИА ---
def safe_send_media(chat_id, file_id, is_document=False, caption=None, reply_markup=None):
    """Универсальная безопасная отправка медиа (картинка или документ)."""
    try:
        if is_document:
            return bot.send_document(chat_id, file_id, caption=caption, parse_mode='HTML', reply_markup=reply_markup)
        else:
        ...
        if "DOCUMENT_INVALID" in e.description or "wrong file identifier" in e.description:
            # Если возникла ошибка из-за несоответствия типа, пробуем отправить противоположным методом
            try:
                if is_document:
                    return bot.send_photo(chat_id, file_id, caption=caption, parse_mode='HTML', reply_markup=reply_markup)
                else:
                ...
                print(f"Критическая ошибка отправки медиа [{file_id}]: {ex}")
                return None
        else:
            print(f"Ошибка Telegram API: {e}")
            return None
    except Exception as e:
        print(f"Неизвестная ошибка при отправке медиа: {e}")
        return None
...
def save_qr(file_id, is_document=0):
    with sqlite3.connect('ggkassa_main.db', timeout=10) as conn:
        c = conn.cursor()
        c.execute('INSERT INTO qr_codes (file_id, is_document, date) VALUES (?, ?, ?)', 
                  (file_id, is_document, datetime.now().strftime("%d.%m.%Y %H:%M")))
        conn.commit()

def get_last_qr():
    with sqlite3.connect('ggkassa_main.db', timeout=10) as conn:
        c = conn.cursor()
        c.execute('SELECT file_id, is_document FROM qr_codes ORDER BY id DESC LIMIT 1')
        row = c.fetchone()
        if row:
...
def get_amount(msg):
    ...
    # Безопасная отправка QR-кода с защитой от DOCUMENT_INVALID
    qr_file_id, is_document = get_last_qr()
    if qr_file_id:
        caption_text = f"{EMOJI['wallet']} <b>ОПЛАТИТЕ {amount:,.2f} сом</b>\n{EMOJI['clock']} 5 минут на оплату"
        sent_msg = safe_send_media(msg.chat.id, qr_file_id, is_document=is_document, caption=caption_text)
        if not sent_msg:
            bot.send_message(msg.chat.id, f"{EMOJI['qr']} Оплатите по реквизитам. (QR-код временно обновляется)", parse_mode='HTML')
    else:
...
def get_check_photo(msg):
    ...
    # Поддержка загрузки чека как фото или как документа
    photo_id = None
    is_doc = False
    ...
    for admin in admins:
        safe_send_media(admin, photo_id, is_document=is_doc, caption=caption_admin, reply_markup=markup)
...
def withdraw_get_elqr(msg):
    ...
    temp_data[msg.chat.id]["is_doc"] = is_doc
...
def withdraw_get_code(msg):
    ...
    is_doc = temp_data.get(user_id, {}).get("is_doc", False)
    ...
    for admin in admins:
        safe_send_media(admin, elqr, is_document=is_doc, caption=caption_admin, reply_markup=markup)
...
def change_qr(msg):
    bot.send_message(msg.chat.id, f"{EMOJI['qr']} Отправьте новый QR-код (фотографию или файл):", parse_mode='HTML', reply_markup=back_menu())
...
def save_new_qr(msg):
    ...
    if file_id:
        save_qr(file_id, is_document=is_doc)
        bot.send_message(msg.chat.id, f"{EMOJI['check']} QR-код успешно обновлен в базе!", parse_mode='HTML', reply_markup=admin_menu())
    else:
        bot.send_message(msg.chat.id, f"{EMOJI['cross']} Пожалуйста, отправьте изображение или документ с QR-кодом!", parse_mode='HTML', reply_markup=back_menu())
        bot.register_next_step_handler(msg, save_new_qr)
...
def view_requests(msg):
    ...
    for dep in deposits:
        ...
        safe_send_media(msg.chat.id, photo_id, is_document=False, caption=caption_text, reply_markup=markup)
...
# --- БЕЗОПАСНЫЙ ЗАПУСК ИСКЛЮЧАЮЩИЙ CONFLICT И DOCUMENT_INVALID ---
def run_bot():
