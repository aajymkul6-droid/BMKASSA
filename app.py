...
# Обязательно впишите СВОЙ ID вместо этих цифр, чтобы кнопки работали!
...
def start(msg):
    ...
    # Сначала ЖЕЛЕЗНО заносим юзера в БД с его реферером, чтобы рефералы не терялись!
    ...
    # Только ПОСЛЕ сохранения проверяем подписку
...
@bot.message_handler(func=lambda m: m.text == "⚙️ Админ" and (m.from_user.id in get_admins() or m.from_user.id == MAIN_ADMIN))
def admin_panel(msg):
...
def get_account_id(msg):
    ...
    # ИСПРАВЛЕН ТЕКСТ: ПОПОЛНЕНИЕ ОТ 100 СОМ
...
def get_amount(msg):
    ...
    # ИСПРАВЛЕНО ОГРАНИЧЕНИЕ: МИНИМУМ 100 СОМ
...
def handle_call(call):
    if call.data == "check_sub":
        if check_subscription(call.message.chat.id):
            ...
            # Передаем оригинальное сообщение
            start(call.message)
        else:
    ...
    # ЖЕЛЕЗНАЯ ПРОВЕРКА ПРАВ ДЛЯ ВСЕХ ИНЛАЙН-КНОПОК АДМИНА
