...
BOT_USERNAME = "BMKASSA_bot"  # Ник бота изменен на BMKASSA_bot
    ...
    if status == "approved":
        ...
        if dep:
            ...
            if ref and ref[0]:
                ...
                try:
                    bot.send_message(ref[0], f"💰 Вам начислено реферальное вознаграждение {bonus:.2f} сом за пополнение друга!")
                except:
                    pass
    conn.commit()
    conn.close()
...
def start(msg):
    ...
    # Если пользователь новый и пришел по реф. ссылке, уведомляем пригласителя
    if is_new and referrer_id:
        try:
            ref_username = f"@{msg.from_user.username}" if msg.from_user.username else msg.from_user.first_name
            bot.send_message(referrer_id, f"➕ У вас новый реферал: {ref_username}")
        except:
            pass

    if not check_subscription(msg.chat.id):
        bot.send_message(msg.chat.id, "Для доступа к боту нужно подписаться на канал:", reply_markup=subscription_markup())
        return
...
def referrals_menu(msg):
    ...
    text = f"""Приглашай друзей и получай стабильный доход
За каждое пополнение друга ты получаешь 3%.

Приглашай по ссылке: {ref_link}

Баланс для вывода: {balance:.2f} сом"""
    ...
    bot.send_message(msg.chat.id, text, reply_markup=markup, disable_web_page_preview=True)
...
def withdraw_start(msg):
    ...
    instruction = """📌 **Как вывести средства с 1ХБЕТ**
...
💳 **Шаг 1:** Прикрепите ваш **ELQR** дейт (фотографией):"""
    
    bot.send_message(msg.chat.id, instruction, parse_mode='Markdown', reply_markup=back_menu())
...
def withdraw_get_elqr(msg):
    ...
    bot.send_message(msg.chat.id, "📸 **Шаг 2:** Отправьте фото (скриншот) вашего **ID 1xbet**:", parse_mode='Markdown', reply_markup=back_menu())
...
def withdraw_get_id_photo(msg):
    ...
    bot.send_message(msg.chat.id, "✉️ **Шаг 3:** После оформления заявки на 1xBet пришлите полученный **код подтверждения** боту:", parse_mode='Markdown', reply_markup=back_menu())
...
def ref_withdraw_get_amount(msg):
    ...
    bot.send_message(msg.chat.id, "🆔 Введите ваш **ID счета 1xBet** для зачисления реферальных средств:", parse_mode="Markdown", reply_markup=back_menu())
...
def ref_withdraw_get_id(msg):
    ...
    for admin in admins:
        try:
            bot.send_message(admin, f"👥 **ЗАЯВКА НА ВЫВОД РЕФЕРАЛЬНЫХ #{rw_id}**\n\n👤 От: {user_id}\n💰 Сумма: {amount} сом\n🎯 На ID 1xBet: `{target_id}`", parse_mode="Markdown", reply_markup=markup)
        except:
            pass
            
    bot.send_message(user_id, f"✅ Заявка на вывод реферальных {amount} сом на ID {target_id} успешно отправлена операторам! Ожидайте обработки.", reply_markup=main_menu(user_id))
    del temp_data[user_id]
...
def handle_call(call):
    if call.data == "check_sub":
        if check_subscription(call.message.chat.id):
            ...
            bot.send_message(call.message.chat.id, f"🚀 Добро пожаловать!", reply_markup=main_menu(call.message.chat.id))
        else:
    ...
    if call.data == "go_to_main":
        bot.delete_message(call.message.chat.id, call.message.message_id)
        bot.send_message(call.message.chat.id, "🚀 Главное меню:", reply_markup=main_menu(call.message.chat.id))
        return
    ...
    if data.startswith('approve_'):
        ...
        bot.answer_callback_query(call.id, "❌ Отклонено")
        try:
            bot.send_message(u_id, f"❌ Ваша заявка на вывод #{w_id} отклонена оператором. Обратитесь в поддержку: {SUPPORT}")
        except:
            pass
        ...
        if result:
            ...
            try:
                bot.send_message(user_id, f"❌ Ваша заявка на вывод реферальных средств в размере {amount} сом была отклонена оператором.")
            except:
                pass
