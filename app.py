...
def check_subscription(user_id):
    try:
        ...
        # Если бота нет в канале или произошла ошибка, временно пропускаем
        return True
...
# --- ОБНОВЛЕННАЯ ЛОГИКА НАЧИСЛЕНИЯ 3% РЕФЕРАЛЬНЫХ ---
def update_deposit_status(dep_id, status):
    ...
    if status == "approved":
        ...
        if dep:
            u_id, amount = dep
            # 1. Проверяем в БД, есть ли у этого друга referrer_id (пригласитель)
            c.execute('SELECT referrer_id FROM users WHERE chat_id = ?', (u_id,))
            ref = c.fetchone()
            if ref and ref[0]:
                # 2. Бот рассчитывает 3% от суммы пополнения
                bonus = amount * 0.03
                # 3. Прибавляет бонус к балансу пригласившего в БД
                c.execute('UPDATE users SET balance = balance + ? WHERE chat_id = ?', (bonus, ref[0]))
                try:
                    # 4. Отправляет уведомление пригласителю
                    bot.send_message(
                        ref[0], 
...
def start(msg):
    ...
    if not bot_active and msg.from_user.id not in get_admins() and msg.from_user.id != MAIN_ADMIN:
...
def referrals_menu(msg):
    ...
    # Изменено описание системы: теперь указано 3% вместо 5%
...
@app.route('/')
def home():
    return {"status": "ok", "message": "BMKASSA Bot is running"}, 200

# --- СТАБИЛЬНЫЙ ЗАПУСК БОТА ---
def run_bot():
...
if __name__ == "__main__":
    ...
    # Запускаем Flask-вебсервер на порту, предоставленном хостинг-средой
